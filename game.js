...
// 🚨 DEMO MODE: Changed fallback from 0 to 10000 so buyers can test the shop immediately!
let coins = parseInt(localStorage.getItem('blockPuzzleCoins')) || 10000; 
...
// HOME BUTTON LOGIC
// ----------------------------------------------------
function promptHome() { playSound('click'); document.getElementById('home-modal').style.display = 'flex'; }
function cancelHome() { playSound('click'); document.getElementById('home-modal').style.display = 'none'; }
...
const SHAPES = [
    ...
    { matrix: [[1,1,1],[1,1,1],[1,1,1]], color: 'purple' }, 
    { matrix: [[1,0],[1,1]], color: 'orange' }, 
    { matrix: [[1,1,1],[1,0,0],[1,0,0]], color: 'cyan' }, 
    { matrix: [[1,1,1],[0,0,1],[0,0,1]], color: 'cyan' } 
];
...
function moveShape(x, y) { activeShape.style.left = `${x - offsetX}px`; activeShape.style.top = `${y - offsetY}px`; }
